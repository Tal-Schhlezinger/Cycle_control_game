# Cycle Control — AI Implementation Checklist

**Status:** operational checklist derived from `AI_DESIGN_V2.1.1`  
...
5. Keep this file aligned with `AI_DESIGN_V2.1.1`.
...
### 4.2 GreedyBot
- [ ] Implement one-ply GreedyBot
- [ ] Include current score / cycle-node term
- [ ] Include territory or future-reach estimate
- [ ] Include component-size term
- [ ] Include mobility / legal-options term
- [ ] Add deterministic tiebreaking
- [ ] Document the evaluation formula and weights
...
- [ ] GreedyBot exists and is documented
...
## 17. Coverage audit against AI_DESIGN_V2.1.1
...
- [x] **V2.1.1 Section 13 — Training phases (including Phase 1 reward-shaping diversity)**  
  Covered by Sections 10, 11, 12. Phase 1 reward-shaping diversity is fully represented in Section 10.2 and 10.3. Phase 3 two-step PBT is represented in Section 12.1 and 12.2.
...
- [x] Includes a self-audit against `AI_DESIGN_V2.1.1`.
...
2. build Random / Greedy / Search,
3. analyze small and mid boards before trusting RL,
4. build GNN first,
5. build CNN as a comparison baseline,
6. train each with reward-shaping diversity built in from the start — each agent consistent in its own reward, varied across agents,
7. then use mixed-opponent training across both architecture families and shaping profiles,
8. only then consider PBT or search-guided upgrades,
9. evaluate the **game** separately from the **agent**.

That is the minimum disciplined path consistent with V2.1.1.
